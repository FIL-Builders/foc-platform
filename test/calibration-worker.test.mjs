...
test("Worker serves HTML and public evidence endpoints", async () => {
  ...
  assert.match(htmlBody, /function disablePagerActions/);
  assert.match(htmlBody, /state\.pageRequestPending = true;/);
  assert.match(htmlBody, /if \(state\.pageRequestPending\) return;/);
  assert.match(htmlBody, /button\.disabled = true;/);
