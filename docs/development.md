...
`pnpm test:tokenhost` checks the schema/config boundary, manifest metadata, and
request/status/object/usage flow through `src/api/platform-api.mjs`. See
`docs/tokenhost-demo.md` for the adjacent builder command and current upstream
builder gaps.
