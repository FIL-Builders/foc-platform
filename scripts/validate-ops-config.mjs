...
const GENERIC_MNEMONIC_OR_SEED_ENV_PATTERN = /(?:^|_)(?:MNEMONIC|SEED)(?:_|$)/;
const GENERIC_RAW_SECRET_ENV_PATTERN = /(?:^|_)(?:PRIVATE_KEY|SECRET)(?:_|$)/;
const RAW_HEX_SECRET_PATTERN = /^(?:0x)?(?:[0-9a-fA-F]{128}|[0-9a-fA-F]{64})$/;
const TRACKED_SECRET_PATTERN =
  /(?:\b[A-Z0-9_]*(?:PRIVATE_KEY|MNEMONIC|SEED|SECRET)[A-Z0-9_]*\b|["']?(?:privateKey|mnemonic|seed|secret)["']?)\s*[:=]\s*["']?(?:0x)?(?:[0-9a-fA-F]{128}|[0-9a-fA-F]{64})\b/g;
  ...
  const keys = Object.keys(env)
    ...
    .filter(
      (key) =>
        ...
        GENERIC_MNEMONIC_OR_SEED_ENV_PATTERN.test(key) ||
        (GENERIC_RAW_SECRET_ENV_PATTERN.test(key) && RAW_HEX_SECRET_PATTERN.test(String(env[key]))),
    );
  if (keys.length > 0) {
